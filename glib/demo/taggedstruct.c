...
static void populate_store_aux(GtkTreeStore *store, GtkTreeIter *parent, PopplerStructureElementIter *iter)
{
    do {
...
static GtkTreeStore *populate_store(PopplerStructureElementIter *iter)
{
...
{*/
static void pgd_selection_changed(GtkTreeSelection *selection, PgdTaggedStructDemo *demo)
{
    GtkTreeModel *model;
    ...
    gtk_label_set_text(GTK_LABEL(demo->id_value), poppler_structure_element_get_id(element));
    gtk_label_set_text(GTK_LABEL(demo->title_value), poppler_structure_element_get_title(element));
    gtk_label_set_text(GTK_LABEL(demo->lang_value), poppler_structure_element_get_language(element));
    gtk_label_set_text(GTK_LABEL(demo->abbr_value), poppler_structure_element_get_abbreviation(element));
...
GtkWidget *pgd_taggedstruct_create_widget(PopplerDocument *document)
{
    ...
    g_signal_connect(selection, "changed", G_CALLBACK(pgd_selection_changed), demo);

    gtk_box_pack_end(GTK_BOX(hbox), vbox, TRUE, TRUE, 0);
    gtk_widget_show(vbox);

    g_object_weak_ref(G_OBJECT(hbox), (GWeakNotify)pgd_taggedstruct_free, demo);
