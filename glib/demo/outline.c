...
static void build_tree(PopplerDocument *document, GtkTreeModel *model, GtkTreeIter *parent, PopplerIndexIter *iter)
{
    ...
    do {
        ...
        if (action->type == POPPLER_ACTION_GOTO_DEST && action->goto_dest.dest->type == POPPLER_DEST_NAMED) {
            /* TODO */
        }

        gtk_tree_store_append(GTK_TREE_STORE(model), &tree_iter, parent);
        gtk_tree_store_set(GTK_TREE_STORE(model), &tree_iter, OUTLINE_TITLE_COLUMN, markup, OUTLINE_ACTION_TYPE_COLUMN, enum_value->value_name, OUTLINE_EXPAND_COLUMN, expand, OUTLINE_ACTION_COLUMN, action, -1);
...
GtkTreeModel *pgd_outline_create_model(PopplerDocument *document)
{
    GtkTreeModel *model;
    ...
    if (iter) {
        ...
        markup = g_strdup_printf("<span size=\"larger\" style=\"italic\">%s</span>", "The document doesn't contain outline");
...
static void expand_open_links(GtkTreeView *tree_view, GtkTreeModel *model, GtkTreeIter *parent)
{
    ...
    if (gtk_tree_model_iter_children(model, &iter, parent)) {
        do {
            gtk_tree_model_get(model, &iter, OUTLINE_EXPAND_COLUMN, &expand, -1);
            if (expand) {
                GtkTreePath *path;
...
static void pgd_outline_selection_changed(GtkTreeSelection *treeselection, GtkWidget *action_view)
{
    GtkTreeModel *model;
    GtkTreeIter iter;
    ...
    if (gtk_tree_selection_get_selected(treeselection, &model, &iter)) {
        ...
        gtk_tree_model_get(model, &iter, OUTLINE_ACTION_COLUMN, &action, -1);
