...
void TestPassword::password1()
{
    ...
    doc = Poppler::Document::load(QStringLiteral(TESTDATADIR "/unittestcases/Gday garçon - open.pdf"), "", QStringLiteral(u"garçon").toLatin1() );
    QVERIFY( doc );
...
void TestPassword::password1a()
{
    ...
    QVERIFY( doc );
    QVERIFY( doc->isLocked() );
    QVERIFY( !doc->unlock( "", QStringLiteral(u"garçon").toLatin1() ) );
    QVERIFY( !doc->isLocked() );
...
{
    Poppler::Document *doc;
    doc = Poppler::Document::load(QString::fromUtf8(TESTDATADIR "/unittestcases/Gday garçon - owner.pdf"), QStringLiteral(u"garçon").toLatin1(), "" );
    QVERIFY( doc );
    QVERIFY( !doc->isLocked() );
